...
void GetCameraCountje(ViconDataStreamSDK::CPP::Client &myClient)
{
    unsigned int CameraCount = myClient.GetCameraCount().CameraCount;
    std::cout << "Cameras(" << CameraCount << "):" << std::endl;
}
...
// Sleep a little so that we don't lumber the CPU with a busy poll
            ...
            for (unsigned int SubjectIndex = 0; SubjectIndex < SubjectCount; ++SubjectIndex)
            {
                ...
                for (unsigned int MarkerIndex = 0; MarkerIndex < MarkerCount; ++MarkerIndex)
                {
                    ...
                    OutputStream << "      Marker #" << MarkerIndex << ": " << MarkerName << " (" << _Output_GetMarkerGlobalTranslation.Translation[0] << ", "
                    ...
                    if (bReadRayData)
                    {
                        ...
                        if (_Output_GetMarkerRayContributionCount.Result == Result::Success)
                        {
                            ...
                            for (unsigned int ContributionIndex = 0; ContributionIndex < _Output_GetMarkerRayContributionCount.RayContributionsCount;
                                 ++ContributionIndex)
                            {
                                ...
                                OutputStream << "ID:" << _Output_GetMarkerRayContribution.CameraID
                                             << " Index:" << _Output_GetMarkerRayContribution.CentroidIndex << " ";
                            }
                            OutputStream << std::endl;
                        }
                    }
            ...
            for (unsigned int UnlabeledMarkerIndex = 0; UnlabeledMarkerIndex < UnlabeledMarkerCount; ++UnlabeledMarkerIndex)
            {
                ...
                OutputStream << "    Marker #" << UnlabeledMarkerIndex << ": (" << _Output_GetUnlabeledMarkerGlobalTranslation.Translation[0] << ", "
                             << _Output_GetUnlabeledMarkerGlobalTranslation.Translation[1] << ", " << _Output_GetUnlabeledMarkerGlobalTranslation.Translation[2]
            ...
            for (unsigned int LabeledMarkerIndex = 0; LabeledMarkerIndex < LabeledMarkerCount; ++LabeledMarkerIndex)
            {
                ...
                OutputStream << "    Marker #" << LabeledMarkerIndex << ": (" << _Output_GetLabeledMarkerGlobalTranslation.Translation[0] << ", "
                             << _Output_GetLabeledMarkerGlobalTranslation.Translation[1] << ", " << _Output_GetLabeledMarkerGlobalTranslation.Translation[2]
                             << ")" << MarkerTrajIDs.str() << std::endl;
            }
            ...
            for (unsigned int DeviceIndex = 0; DeviceIndex < DeviceCount; ++DeviceIndex)
            {
                ...
                for (unsigned int DeviceOutputIndex = 0; DeviceOutputIndex < DeviceOutputCount; ++DeviceOutputIndex)
                {
                    ...
                    unsigned int DeviceOutputSubsamples =
                        MyClient
                            .GetDeviceOutputSubsamples(_Output_GetDeviceName.DeviceName, _Output_GetDeviceOutputComponentName.DeviceOutputName,
                    ...
                    for (unsigned int DeviceOutputSubsample = 0; DeviceOutputSubsample < DeviceOutputSubsamples; ++DeviceOutputSubsample)
                    {
                        ...
                        Output_GetDeviceOutputValue _Output_GetDeviceOutputValue =
                            MyClient.GetDeviceOutputValue(_Output_GetDeviceName.DeviceName, _Output_GetDeviceOutputComponentName.DeviceOutputName,
                                                          _Output_GetDeviceOutputComponentName.DeviceOutputComponentName, DeviceOutputSubsample);

                        OutputStream << "          '" << _Output_GetDeviceOutputComponentName.DeviceOutputComponentName << "' "
                                     << _Output_GetDeviceOutputValue.Value << " " << Adapt(_Output_GetDeviceOutputComponentName.DeviceOutputUnit) << " "
            ...
            for (unsigned int ForcePlateIndex = 0; ForcePlateIndex < ForcePlateCount; ++ForcePlateIndex)
            {
                ...
                for (unsigned int ForcePlateSubsample = 0; ForcePlateSubsample < ForcePlateSubsamples; ++ForcePlateSubsample)
                {
                    ...
                    Output_GetGlobalMomentVector _Output_GetMomentVector = MyClient.GetGlobalMomentVector(ForcePlateIndex, ForcePlateSubsample);
                    ...
                    Output_GetGlobalCentreOfPressure _Output_GetCentreOfPressure = MyClient.GetGlobalCentreOfPressure(ForcePlateIndex, ForcePlateSubsample);
            ...
            if (bReadCentroids)
            {
                ...
                for (unsigned int CameraIndex = 0; CameraIndex < CameraCount; ++CameraIndex)
                {
                    ...
                    auto retu = MyClient.GetCameraId(CameraName);
                    ...
                    OutputStream << "    Resolution: " << _Output_GetCameraResolution.ResolutionX << " x " << _Output_GetCameraResolution.ResolutionY
                                 << std::endl;
                    ...
                    OutputStream << "    Global Translation: (" << _OutputGlobalTranslation.Translation[0] << ", " << _OutputGlobalTranslation.Translation[1]
                                 << ", " << _OutputGlobalTranslation.Translation[2] << ")" << std::endl;

                    // Get the global segment rotation in helical co-ordinates
                    Output_GetCameraGlobalRotationHelical _Output_GetCameraGlobalRotationHelical = MyClient.GetCameraGlobalRotationHelical(CameraName);
                    OutputStream << "   Global Rotation Helical: (" << _Output_GetCameraGlobalRotationHelical.Rotation[0] << ", "
                                 << _Output_GetCameraGlobalRotationHelical.Rotation[1] << ", " << _Output_GetCameraGlobalRotationHelical.Rotation[2]
                                 << std::endl;

                    // Get the global segment rotation as a matrix
                    Output_GetCameraGlobalRotationMatrix _Output_GetCameraGlobalRotationMatrix = MyClient.GetCameraGlobalRotationMatrix(CameraName);
                    OutputStream << "   Global Rotation Matrix: (" << _Output_GetCameraGlobalRotationMatrix.Rotation[0] << ", "
                                 << _Output_GetCameraGlobalRotationMatrix.Rotation[1] << ", " << _Output_GetCameraGlobalRotationMatrix.Rotation[2] << ", "
                                 << _Output_GetCameraGlobalRotationMatrix.Rotation[3] << ", " << _Output_GetCameraGlobalRotationMatrix.Rotation[4] << ", "
                                 << _Output_GetCameraGlobalRotationMatrix.Rotation[5] << ", " << _Output_GetCameraGlobalRotationMatrix.Rotation[6] << ", "
                                 << _Output_GetCameraGlobalRotationMatrix.Rotation[7] << ", " << _Output_GetCameraGlobalRotationMatrix.Rotation[8] << std::endl;

                    // Get the global segment rotation in quaternion co-ordinates
                    Output_GetCameraGlobalRotationQuaternion _Output_GetCameraGlobalRotationQuaternion = MyClient.GetCameraGlobalRotationQuaternion(CameraName);
                    OutputStream << "   Global Rotation Quaternion: (" << _Output_GetCameraGlobalRotationQuaternion.Rotation[0] << ", "
                                 << _Output_GetCameraGlobalRotationQuaternion.Rotation[1] << ", " << _Output_GetCameraGlobalRotationQuaternion.Rotation[2]
                                 << ", " << _Output_GetCameraGlobalRotationQuaternion.Rotation[3] << std::endl;

                    // Get the global segment rotation in EulerXYZ co-ordinates
                    Output_GetCameraGlobalRotationEulerXYZ _Output_GetCameraGlobalRotationEulerXYZ = MyClient.GetCameraGlobalRotationEulerXYZ(CameraName);
                    OutputStream << "        Global Rotation EulerXYZ: (" << _Output_GetCameraGlobalRotationEulerXYZ.Rotation[0] << ", "
                                 << _Output_GetCameraGlobalRotationEulerXYZ.Rotation[1] << ", " << _Output_GetCameraGlobalRotationEulerXYZ.Rotation[2]
                                 << std::endl;
                    ...
                    if (bReadGreyscaleData)
                    {
                        ...
                        {
                            OutputStream << "   WindowSize( " << _Output_CameraWindowSize.WindowStartX << ", " << _Output_CameraWindowSize.WindowStartY << ", "
                                         << _Output_CameraWindowSize.WindowWidth << ", " << _Output_CameraWindowSize.WindowHeight << " ):" << std::endl;
                        }
                        ...
                        {
                            OutputStream << "   Subsampling( " << _Output_SubsampleInfo.TwiceOffsetX << ", " << _Output_SubsampleInfo.TwiceOffsetY << ", "
