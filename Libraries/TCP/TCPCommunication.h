...
bool Ping(const char* ipAddress, DWORD iTimeout = 1000);
...
*/
//--------------------------------------------------------------------------------------------------------------------------------------
bool ResolveIP4_Address(LPCSTR HostName,CString& IP_Number); // returns true on succes
...
//Codes for tcpgrams
const unsigned char TCPGRAM_CODE_INVALID(0);		// array of doubles
const unsigned char TCPGRAM_CODE_DOUBLES(1);		// array of doubles
const unsigned char TCPGRAM_CODE_COMMAND(2);		// CNode command
const unsigned char TCPGRAM_CODE_STATUS(3);		// CNode status
const unsigned char TCPGRAM_CODE_STRING(4);		// text string, includes 0-terminator
const unsigned char TCPGRAM_CODE_INTERRUPT(5);	// interrupt send by client, when received in the TCP communication thread, this will directly set an interrupt field in StateManager
const unsigned char TCPGRAM_CODE_WARNING(6);		// contains a warning
...
public: // various 
	...
	CTCPGram(double* pDoubles, int NumDoubles);
	CTCPGram(std::vector<double>& arDoubles);
	CTCPGram(cliext::vector<double>^ arDoubles);
	CTCPGram(cliext::deque<double>^ queDoubles);
	CTCPGram(CString& rString,unsigned char Code = TCPGRAM_CODE_STRING);
	CTCPGram(char* pBytes, unsigned long NumBytes);
	CTCPGram(TiXmlElement* rCommand, unsigned char Code = TCPGRAM_CODE_COMMAND);
...
public:
	...
	virtual unsigned long					GetNumDoubles();
	virtual bool							GetDoubles(double* pVector,int iNumDoubles); // pDoubles should be pre-allocated with iNumDouble doubles, false is returned if the sizes don't match
	virtual bool							GetString(CString& rString);
	virtual void							Clear();
public:
	...
	SOCKET					m_Source;			// socket from where this tcpgram came
};
...
enum E_COMMUNICATION_Mode{TCP_SERVER,UDP,TCP_CLIENT};
...
{
public:
	CSocket(SOCKET iSocket,E_COMMUNICATION_Mode, SOCKADDR_IN* ipSockAddress, int iUDPReceivePort, bool iUDPBroadcast, LPCTSTR iUDPSendToAddress);
	virtual ~CSocket();
...
public: 
	...
	virtual void		ReadFeedBuffer();            // appends read data, produces a CTCPGram when the complete package is received, throws FALSE if connection was reset or CExceptionSocket for socket error
	...
	virtual bool		WriteSend(std::unique_ptr<CTCPGram>&);       //continues writing packets of the CTCPGram, when the complete TCPGram has been transmitted, then true is returned, throws FALSE if connection was reset or CExceptionSocket for socket error
...
protected: // read buffer
	unsigned long			m_CurrentTelegramSize = 0;              // the size of a telegram is in the front of a message, that is set here when a new message is started for reception
...
public:
	...
	virtual void			SetError(LPCTSTR iErrorString);
public: // CCommunicationParameters
	virtual CSocket*		GetNewComer();// pops socket of oldest client that recently connected to our server, returns INVALID_SOCKET if no new sockets are available
	virtual void			AddNewComer(CSocket*);
	virtual int				GetNumConnections(){return 0;};
...
public:// sending data : pushes a telegram into the FIFO stack
	virtual bool			GetSendPackage(std::unique_ptr<CTCPGram>&);					// thrd: pops the oldest package from the start of the list, the returned CTCPGram needs to be destroyed afterwards
	virtual bool			GetReceivePackage(std::unique_ptr<CTCPGram>&);				// Object: pops the oldest package from the start of the list, the returned CTCPGram needs to be destroyed afterwards
...
protected: // sockets and states
	...
	CString					m_HostName;			                        // hostname, like "ctech-tower", "127.0.0.1" "localhost" "www.ctechmetrology.com"
...
protected:// error handling
	...
	CString					m_ErrorString;		// error that caused our communication to stops
protected:// buffers
	std::list< std::unique_ptr<CTCPGram>>	m_arSendBuffer;
	std::list< std::unique_ptr<CTCPGram>>	m_arReceiveBuffer;
	std::list<CSocket*>						m_arNewComers;		// list of sockets that newly connected to our server socket, used to send a configuration to the newly connected socket when the engine is running
...
public: // open close state
	bool				IsOpen();
	void				Open();
	void				Open(E_COMMUNICATION_Mode iTcpMode, int iPort, int iPortUDP = 0, LPCTSTR iIpAddress = _T("127.0.0.1"));
	void				Close();
...
public: // own overrideable functions
	virtual CSocket*	SocketCreate(SOCKET iSocket, E_COMMUNICATION_Mode, SOCKADDR_IN* ipSockAddress, unsigned short UDPReceivePort, bool UDPBroadcast, LPCTSTR UDPSendPort);//CSocket* ipSocket,bool bAddToNewComerList = false);public: // get set ip4 related stuff
protected:
	std::unique_ptr<CCommunicationThread>	m_pCommunicationThread;
...
public: // CCommunicationParameters
	...
	void			SetError(LPCTSTR iErrorString) override;
protected:
	void			SocketAdd(SOCKET iSocket,E_COMMUNICATION_Mode, SOCKADDR_IN* ipSockAddress,unsigned short UDPReceivePort, bool bAddToNewComerList,bool UDPBroadcast, LPCTSTR UDPSendPort);//CSocket* ipSocket,bool bAddToNewComerList = false);
...
protected: // thread management
	CWinThread*						m_pExecuteThread = nullptr;
	volatile bool					m_bQuit		= false;	// set to true to end the communication thread
