...
bool Ping(const char* HostName, DWORD iTimeout)
{
...
bool ResolveIP4_Address(LPCSTR HostName,CString& IP_Number)
{
	// local host spotted, return true
	IP_Number = HostName;
	if (IP_Number.CompareNoCase(_T("127.0.0.1"))==0)
		return true;
...
CTCPGram::CTCPGram(CString& rString,unsigned char Code)
{
	m_Destination	= ALL_DESTINATIONS;
	m_Source		= 0;
...
CTCPGram::CTCPGram(TiXmlElement* pCommand, unsigned char Code)
{
	if (pCommand)
	{
		CString XMLText;
...
CString	CTCPGram::GetDescription()
{
	CString ReturnString;
	int Code = GetCode();
	switch (Code)
	{
	case TCPGRAM_CODE_INVALID: ReturnString = "Received INVALID"; break;
	case TCPGRAM_CODE_DOUBLES: ReturnString = "Received DOUBLES"; break;
	case TCPGRAM_CODE_COMMAND: ReturnString = "Received COMMAND"; break;
	case TCPGRAM_CODE_STATUS: ReturnString = "Received STATUS"; break;
	case TCPGRAM_CODE_STRING: ReturnString = "Received STRING"; break;
	case TCPGRAM_CODE_INTERRUPT: ReturnString = "Received INTERRUPT"; break;
	case TCPGRAM_CODE_WARNING: ReturnString = "Received WARNING"; break;
	default:
		break;
	}
	return ReturnString;
...
std::unique_ptr<TiXmlElement>	CTCPGram::GetXML()
{
	...
	std::unique_ptr<TiXmlElement> ReturnXML(pXML);
	return ReturnXML;
}
...
bool				CTCPGram::GetString(CString& rString)
{
	unsigned char	Code			= GetCode();
	unsigned long	TeleGramsSize	= GetSize();
...
CString			CCommunicationInterface::GetError()
{
	CSingleLock Lock(&m_Lock, true);
...
CString			CCommunicationInterface::GetHostName()
{
	CSingleLock Lock(&m_Lock, true);
	return m_HostName;
}
...
bool CCommunicationInterface::EditSettingsChanged()
{
	...
	{
		return (m_CommunicationMode != static_cast<E_COMMUNICATION_Mode>(m_pPropertyPage->m_ComboDataType) ||
...
CSocket*	CCommunicationObject::SocketCreate(SOCKET iSocket, E_COMMUNICATION_Mode Mode, SOCKADDR_IN* ipSockAddress, unsigned short UDPBroadCastPort, bool UDPBroadcast, LPCTSTR UDPSendAddress)
{
...
void CCommunicationObject::Open()
{
	...
	{
		// when a communication object wants to open the communication channels, CCommunicationManager checks if there is already a thread running on the same port, if yes then the communication object is added
		// to the list of communication objects belonging to that port, if no, then we create a new thread
...
CSocket::CSocket(SOCKET iSocket,E_COMMUNICATION_Mode iCommunicationMode, SOCKADDR_IN* ipSockAddress, int UDBroadCastPort, bool iUDPBroadcast, LPCTSTR iUDPSendToAddress)
{
	m_Socket			    = iSocket;
...
bool	CSocket::ReadExtractTelegram(std::unique_ptr<CTCPGram>& ReturnTCPGram)
{
	...
	{
		ReturnTCPGram.reset(new CTCPGram(RemoveFront(m_pAccumulationBuffer,m_AccumulationBufferSize,m_CurrentTelegramSize))); // remove front returns the split of buffer which is then coupled directly with the telegram, who will finally destroy it
	...
	case TCP_SERVER:
		{
			...
			int NumBytesWritten = send(m_Socket,&(rTCPGram->m_pData.get()[m_TotalBytesWritten]),MIN(RECEIVEBUFFERLENGTH, rTCPGram->m_PackageSize-m_TotalBytesWritten),0);
	...
	case UDP:
		{
			...
			{
				CString ErrorMessage;
				ErrorMessage.Format(_T("Error sending data over UDP : the package size (%d) is bigger than allowed (%d)"), rTCPGram->m_PackageSize,m_MaxUDPMessageSize);
				THROW_ERROR(ErrorMessage);
			}
...
void	CCommunicationThread::SocketAdd(SOCKET iSocket, E_COMMUNICATION_Mode Mode, SOCKADDR_IN* ipSockAddress, unsigned short UDPBroadCastPort, bool bAddToNewComerList, bool UDPBroadcast, LPCTSTR UDPSendPort)
{
	CSingleLock Lock(&m_Lock, true);
...
void			CCommunicationThread::EndThread()
{
	...
	if (m_pExecuteThread)
	{
		...
		switch (nResult)
		{
			case WAIT_OBJECT_0: break; // success
...
{	
	...
	try
	{
		...
		//--------------------------------------------------------------------------------------------------------
		CString			IP4;
		CString			HostName		= pTCPParameters->GetHostName();
		...
		case TCP_SERVER:
		{
			...
			if (::listen(MainSocket, 1/*backlog amount permitted*/) == SOCKET_ERROR)
			{
				int LastError = WSAGetLastError();
				CString ErrorMessage;
				ErrorMessage.Format(_T("The listen command on the socket (host : %s port:%d) failed.Lasterror was %d.\nPossibly the port is already in use."), HostName, PortNumber,LastError);
		...
		case UDP: // udp is connectionless, so it cannot be disconnected, so no need to check this in the big loop
			{
				...
				if (::bind(MainSocket, (LPSOCKADDR)&sincontrol, sizeof(sincontrol)) == SOCKET_ERROR)
				{
					int LastError = WSAGetLastError();
					CString ErrorMessage;
					ErrorMessage.Format(_T("The binding of the socket (host : %s port:%d) failed.Lasterror was %d.\nPossibly the port is already in use."), HostName, PortNumberUDP, LastError);
					std::cout << ErrorMessage << std::endl;
					AfxMessageBox(ErrorMessage, MB_ICONERROR);
					THROW_SOCKET_ERROR(ErrorMessage, false);
				}
			...
			case TCP_SERVER:
				{
					...
					bool	bDataAvail(false),bNetWorkError(false);
					long	FAR errval(0);
					DataAvailable(MainSocket,bDataAvail,bNetWorkError,errval); // a client trying to connect on our server will send data to the listen socket
					if (bDataAvail)
					{
