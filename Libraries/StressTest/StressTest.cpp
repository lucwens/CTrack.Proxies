...
StressTest::StressTest(CTrack::IDriver* driver, std::shared_ptr<CTrack::MessageResponder> responder)
    ...
    , m_DeviceName(driver ? driver->GetDeviceName() : "Unknown")
    , m_PollingIntervalMs(driver ? driver->GetRecommendedPollingIntervalMs() : 20)
    , m_RandomEngine(std::random_device{}())
    , m_ActionDistribution(0, 6) // 7 possible actions
{
}
...
void StressTest::Start()
{
    ...
    LogInfo(fmt::format("Device: {}", m_DeviceName));
    if (m_pDriver)
    {
        std::string deviceInfo = m_pDriver->GetDeviceInfo();
        if (!deviceInfo.empty())
        {
            LogInfo(fmt::format("Device Info: {}", deviceInfo));
        }
    }
    LogInfo(fmt::format("Purpose: Test {} communication stability", m_DeviceName));
    LogInfo(fmt::format("Polling interval: {}ms ({} Hz)", m_PollingIntervalMs, 1000 / m_PollingIntervalMs));
...
bool StressTest::DoHardwareDetect()
{
    ...
    std::string feedback;
    bool present = m_pDriver->HardwareDetect(feedback);
    ...
    if (!present)
    {
        ...
        std::string lastError = m_pDriver->GetLastError();
        if (!lastError.empty())
        {
            LogWarning(fmt::format("Last error: {}", lastError));
        }
        return false;
    }
    return true;
...
bool StressTest::DoConfigDetect()
{
    ...
    std::string feedback;
    bool success = m_pDriver->ConfigDetect(feedback);

    if (success)
    {
        LogInfo(fmt::format("Configuration: {}", feedback));
        return true;
    }

    LogError(fmt::format("Config detect failed: {}", feedback));
    std::string lastError = m_pDriver->GetLastError();
    if (!lastError.empty())
    {
        LogError(fmt::format("Last error: {}", lastError));
    }
    return false;
}
...
bool StressTest::DoStartTracking()
{
    ...
    PrintInfo(fmt::format("STRESS TEST [{}]: Start Tracking", m_DeviceName));

    // Calculate frequency from polling interval
    double frequencyHz = 1000.0 / m_PollingIntervalMs;

    bool result = m_pDriver->Initialize(frequencyHz);
    ...
    if (result)
    {
        ...
        LogInfo(fmt::format("Tracking started successfully at {:.0f} Hz - will track for {} seconds before allowing stop",
                           frequencyHz, m_RequiredMeasurementSeconds));
        return true;
    }
    else
    {
        std::string lastError = m_pDriver->GetLastError();
        LogError(fmt::format("Failed to start tracking: {}", lastError.empty() ? "Unknown error" : lastError));
        return false;
    }
}
...
bool StressTest::DoStopTracking()
{
    ...
    PrintInfo(fmt::format("STRESS TEST [{}]: Stop Tracking", m_DeviceName));

    bool result = m_pDriver->Shutdown();
    ...
    {
        std::string lastError = m_pDriver->GetLastError();
        LogError(fmt::format("Failed to stop tracking: {}", lastError.empty() ? "Unknown error" : lastError));
        return false;
    }
}
...
void StressTest::DoWait(int minSeconds, int maxSeconds)
{
    ...
    PrintInfo(fmt::format("STRESS TEST [{}]: Waiting {} seconds", m_DeviceName, waitSeconds));
    ...
    // Calculate frames per second based on device polling interval
    int framesPerSecond = 1000 / m_PollingIntervalMs;
        ...
        if (m_bCurrentlyTracking && m_pDriver->IsRunning())
        {
            ...
            if (elapsedSeconds % 10 == 0)  // Log every 10 seconds to avoid spam
            {
                LogInfo(fmt::format("Processed {} Run() calls in last second (FPS: {:.1f})",
                                   framesProcessed, m_pDriver->GetCurrentFPS()));
            }
        }
        else
        {
...
void StressTest::InitLogFile()
{
    ...
    m_LogFilePath = (logDir / fmt::format("{}_StressTest_{}.log", m_DeviceName, timestamp.str())).string();
